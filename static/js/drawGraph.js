...
function draw(radius) {
    ...
    drawCircle(ctx, 2, centerX, centerY, R / 2);
    drawRectangle(ctx, 3, centerX, centerY, R, R / 2, R);
    drawTriangle(ctx, 1, centerX, centerY, R, R / 2);
    drawCoordinateGrid(ctx, centerX, centerY, width, height, step);

    if (!isNaN(R) || R !== null) {
        canvas.onclick = e => {
            handleSubmit(Math.round((e.offsetX - centerX) / step), (centerY - e.offsetY) / step).then();
        }
    } else {
